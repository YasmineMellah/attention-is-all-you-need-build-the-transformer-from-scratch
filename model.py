...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    for word in sentence.split():
        res.append(token_to_id.get(word, token_to_id[unk_token]))
    return res
...
def pad_id_sequence(ids, max_len, pad_id):
    len_ids = len(ids)
    if len_ids < max_len:
        len_diff = max_len - len_ids
        return ids + [pad_id] * len_diff
    return ids[:max_len]
...
def add_positional_encoding_to_embeddings(embedded_batch, positional_encoding):
    ...
    embedded_batch = positional_encoding.unsqueeze(0)[:, :L, :] + embedded_batch
...
def mask_attention_scores_with_neg_inf(scores, mask):
    ...
    mask = scores.masked_fill(~mask, float('-inf'))
    return mask
...
def softmax_attention_weights(masked_scores):
    ...
    weights = torch.softmax(masked_scores.masked_fill(all_masked, 0.0), dim=-1)
...
def scaled_dot_product_attention(query, key, value, mask=None):
    ...
    scores = scale_attention_scores(scores, query.size(-1))
    if mask is not None:
...
def normalize_and_scale_with_gamma_beta(x, gamma, beta, eps=1e-5):
    ...
    standarized_x = (x - mean) / torch.sqrt(var + eps) 
    return gamma * standarized_x + beta
